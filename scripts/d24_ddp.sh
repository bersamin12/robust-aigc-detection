...
"$PY" -m torch.distributed.run --nproc_per_node="$NPROC" --master_port=29577 \
  ...
  --out-dir "$OUT" --epochs "$EPOCHS" --src-chunk "$CHUNK" \
...
# --- stage 3: the full ladder, each rung on the bank its own tower produced --
for kind in ov7 wf; do
  base=data/banks/eval_ov7_d; outj=docs/selection_ov7_unfreeze_d24.json
  [ "$kind" = wf ] && { base=data/banks/eval_unfreeze_d; outj=docs/selection_wf_unfreeze_d24.json; }
  declare -a R; R=()
  for d in 0 4 6 8 24; do
    [ -f "${base}${d}/config.json" ] || continue
    ck=$OUT/d${d}/checkpoint.pt
    [ "$d" = 0 ] && [ "$kind" = ov7 ] && ck=$OUT/d0snap/checkpoint.pt
    [ -f "$ck" ] && R+=(--rung "d${d}=${ck}:${base}${d}")
  done
...
say "=== D24 COMPLETE in $(( ($(date +%s)-T0)/60 )) min"
